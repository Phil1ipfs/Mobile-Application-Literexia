...
class PlayHTService {
  static const MethodChannel _channel = MethodChannel('com.example.literexia/playht');
  ...
  Future<List<Map<String, dynamic>>> getFilipinoVoices() async {
    ...
    return voices.where((voice) {
      ...
      final languageCode = voice['language_code']?.toString().toLowerCase() ?? '';
  ...
  Future<Map<String, dynamic>?> getVoiceByName(String name) async {
    ...
    try {
      return voices.firstWhere(
        (voice) => voice['name']?.toString().toLowerCase() == name.toLowerCase()
      );
    } catch (e) {
      return null;
    }
  ...
  Future<List<Map<String, dynamic>>> getVoicesByLanguage(String languageCode) async {
    final voices = await getVoices();
