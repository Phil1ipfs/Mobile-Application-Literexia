...
class CategoryValidationHelper {
  ...
  static void testCategoryMapping() {
    ...
    final testCases = [
      ...
      {
        'questionId': 'main_q_1',
        'expectedCategory': 'Should use current assessment category'
      },
    ];
  ...
  static bool validateAssessmentData(Map<String, dynamic> assessmentData) {
    ...
    if (!assessmentData.containsKey('studentId') ||
        assessmentData['studentId'] == null) {
      errors.add('Missing required field: studentId');
      isValid = false;
    }

    if (!assessmentData.containsKey('categories') ||
        assessmentData['categories'] == null) {
      ...
      if (categories == null || categories.isEmpty) {
        ...
        for (int i = 0; i < categories.length; i++) {
          ...
          if (!categoryMap.containsKey('score') ||
              categoryMap['score'] == null) {
            errors.add('Category at index $i missing score');
            isValid = false;
          }

          if (!categoryMap.containsKey('totalQuestions') ||
              categoryMap['totalQuestions'] == null) {
            errors.add('Category at index $i missing totalQuestions');
            isValid = false;
          }

          if (!categoryMap.containsKey('correctAnswers') ||
              categoryMap['correctAnswers'] == null) {
  ...
  static Map<String, dynamic> generateTestAssessmentData({
    ...
    final categories = VALID_CATEGORIES.map((categoryName) {
      final totalQuestions =
          isPreAssessment ? 1 : 5; // Pre-assessment has 1 question per category
    ...
    final overallScore =
        categories.fold<int>(0, (sum, cat) => sum + (cat['score'] as int)) ~/
            categories.length;
    ...
    if (!isPreAssessment) {
      testData['primaryCategory'] =
          VALID_CATEGORIES.first; // Use first category as primary
    }
  ...
  static void debugQuestionCategoryMapping(
      List<dynamic> questions, String currentCategory) {
...
class DatabaseService {
  ...
  Future<Db> getPreAssessmentDatabase() async {
    ...
    print(
        '[DatabaseService] Connecting directly to Pre_Assessment database ➜ ${_maskUri(uriWithDb)}');
  ...
  Future<bool> initialize() async {
    ...
    try {
      ...
      print(
          '[DatabaseService] Connecting to default database ➜ ${_maskUri(uri)}');
      ...
      _isInitialized = true;
      print(
          '[DatabaseService] Connected to default database: ${_db!.databaseName}');
      print(
    ...
    try {
      ...
      if (syncCompletionData && isConnected && _db != null) {
        try {
          ...
          print(
              '[DatabaseService] Synced lesson completion data from MongoDB for user $idNumber');
        } catch (e) {
  ...
  DbCollection getCollection(String name) {
    ...
    if (!isConnected) {
      print(
          '[DatabaseService] WARNING: Connection is not active, operations may fail');
  ...
  Future<bool> _reconnectAsync() async {
    try {
      ...
      if (mongoUri != null) {
        ...
        print(
            '[DatabaseService] Error: MongoDB URI not found in environment variables');
        return false;
      }
    } catch (e) {
...
// Enhanced getLessonsForLevel method with strict reading level filtering
  Future<List<Map<String, dynamic>>> getLessonsForLevel(
    ...
    print(
        '[DatabaseService] Getting lessons for EXACT level: $targetReadingLevel');
    ...
    try {
      if (isConnected && _db != null) {
        ...
        print(
            '[DatabaseService] Querying main_assessment for EXACT readingLevel: $targetReadingLevel');
        ...
        print(
            '[DatabaseService] Found ${assessments.length} assessments for EXACT level $targetReadingLevel');

        // If no exact matches found, DO NOT fallback to other levels
        if (assessments.isEmpty) {
          print(
              '[DatabaseService] No assessments found for reading level $targetReadingLevel');
          print(
              '[DatabaseService] Will NOT fallback to other levels for content integrity');
        ...
        for (final assessment in assessments) {
          print(
              '[DatabaseService] Assessment found: Category=${assessment['category']}, Level=${assessment['readingLevel']}, Questions=${(assessment['questions'] as List?)?.length ?? 0}');
        }
        ...
        assessments = assessments.where((assessment) {
          ...
          if (!isMatch) {
            print(
                '[DatabaseService] WARNING: Found assessment with mismatched level: $assessmentLevel vs $targetReadingLevel');
          }
        ...
        print(
            '[DatabaseService] User $safeUserIdNumber has completed lessons: $safeCompletedLessons');
        ...
        for (final assessment in assessments) {
          ...
          if (normalizedAssessmentLevel != targetReadingLevel) {
            print(
                '[DatabaseService] Skipping assessment with mismatched level: $assessmentLevel vs $targetReadingLevel');
            continue;
          }
          ...
          final bool isCompleted = safeUserIdNumber.isNotEmpty
              ? await hasStudentCompletedAssessment(
                  safeUserIdNumber, assessmentIdString)
              : false;
          ...
          if (index > 1 &&
              ...
              safeUserIdNumber.isNotEmpty) {
            // Check if previous lesson is completed OR if this lesson was explicitly made available
            final previousAssessmentId =
                assessments[index - 2]['_id'].toString();
            final previousCompleted = await hasStudentCompletedAssessment(
                safeUserIdNumber, previousAssessmentId);
            ...
            final isExplicitlyAvailable =
                await _isLessonExplicitlyAvailable(safeUserIdNumber, index);
          ...
          final questionCount =
              (assessment['questions'] as List<dynamic>?)?.length ?? 5;
          ...
          lessons.add({
            ...
            'description':
                _getDescriptionForLevel(targetReadingLevel, category),
          ...
          print(
              '[DatabaseService] Created lesson $index: $category (Available: $isAvailable, Completed: $isCompleted, Level: $targetReadingLevel)');
          index++;
        }
        ...
        print(
            '[DatabaseService] Returning ${lessons.length} lessons for reading level $targetReadingLevel');
        return lessons;
      }
      ...
      print(
          '[DatabaseService] Error fetching lessons for level $targetReadingLevel: $e');
  ...
  String _normalizeReadingLevel(String readingLevel) {
    ...
    if (lowercaseLevel.contains('low') && lowercaseLevel.contains('emerg')) {
      ...
      print(
          '[DatabaseService] WARNING: Unrecognized reading level format: "$inputLevel"');
    }

    if (normalizedLevel != inputLevel) {
      print(
          '[DatabaseService] Normalized reading level: "$inputLevel" → "$normalizedLevel"');
    } else {
      print(
          '[DatabaseService] Reading level is already normalized: "$normalizedLevel"');
    }
  ...
  Future<void> _saveLessonsToLocalDb(
      List<Map<String, dynamic>> lessons, String readingLevel) async {
    ...
    try {
      ...
      print(
          '[DatabaseService] Saved ${lessons.length} lessons for level $readingLevel to local DB (without category column)');
    } catch (e) {
  ...
  Future<List<Map<String, dynamic>>> _getLessonsFromLocalDb(
    ...
    try {
      ...
      if (localLessons.isNotEmpty) {
        print(
            '[DatabaseService] Found ${localLessons.length} lessons in local DB for level $readingLevel');
        ...
        return localLessons
            .map((lesson) => {
                  ...
                  'assessmentId': lesson['assessmentId'] ??
                      'local_${lesson['lessonIndex']}',
                  'readingLevel': readingLevel, // Ensure consistency
                  'category': lesson['category'] ??
                      'Filipino Lesson', // IMPORTANT: Preserve category
                })
            .toList();
      }

      print(
          '[DatabaseService] No local lessons found for reading level: $readingLevel');
      return [];
    } catch (e) {
  ...
  Future<void> markLessonAsCompleted(
      String userIdNumber, int lessonIndex) async {
    try {
      ...
      if (isConnected && _db != null) {
        ...
        if (user != null) {
          ...
          if (!completedLessons.contains(lessonIndex) &&
            ...
            print(
                'Marked lesson $lessonIndex as completed for user $userIdNumber in MongoDB');
          } else {
            print(
                'Lesson $lessonIndex already completed for user $userIdNumber');
          }
        } else {
      ...
      if (_localDb != null) {
        ...
        if (existingRecord.isEmpty) {
          ...
          print(
              'Marked lesson $lessonIndex as completed for user $userIdNumber in local DB');
        } else {
          print(
              'Lesson $lessonIndex already completed for user $userIdNumber in local DB');
        }
      }
    } catch (e) {
    ...
    try {
          ...
          for (final coll in paCollections) {
            ...
            if (coll == 'pre-assessment') {
              ...
              if (count > 0) {
                ...
                final sample =
                    await _preAssessmentDb!.collection(coll!!).findOne();
                if (sample != null) {
                  ...
                  if (sample.containsKey('questions')) {
                    print(
                        '    Questions count: ${(sample['questions'] as List?)?.length ?? 0}');
                  } else {
      ...
      if (_db != null) {
        try {
          ...
          print(
              '  In default database (${_db!.databaseName}): $count documents');
        } catch (e) {
      ...
      try {
        ...
        final result = await preAssessmentDb
            .collection('pre-assessment')
            .count(where.eq('assessmentId', 'FL-G1-001'));
        print('  Documents with assessmentId=FL-G1-001: $result');

        // First try to find the document from paste.txt
        final checkDoc = await preAssessmentDb
            .collection('pre-assessment')
            .findOne(where.eq('assessmentId', 'FL-G1-001'));
        if (checkDoc != null) {
          print('  Found test document from paste.txt');
        } else {
          // If document doesn't exist, insert it
          print(
              '  Document from paste.txt not found - you may need to insert it manually');
  ...
  Future<void> saveStudentResponse(Map<String, dynamic> response) async {
    ...
    try {
      ...
      if (response['studentId'] != null) {
        ...
        if (currentStudentId is String) {
          try {
            ...
            print(
                '[DatabaseService] Converted studentId to integer: $intStudentId');
          } catch (e) {
            print(
                '[DatabaseService] WARNING: Could not convert studentId to integer: $e');
          }
        }
      }
      ...
      print(
          '[DatabaseService]   - Student ID: $studentId (${studentId.runtimeType})');
      ...
      if (result.isSuccess) {
        print(
            '[DatabaseService] Successfully saved student response with integer studentId');
      } else {
        print(
            '[DatabaseService] Failed to save student response: ${result.writeError?.errmsg}');
      }
    } catch (e) {
  ...
  Future<String> saveCategoryResult(Map<String, dynamic> result) async {
    if (!isConnected || _db == null) {
      print(
          '[DatabaseService] Cannot save category result - not connected to DB');
      return '';
    }
    ...
    try {
      ...
      if (insertResult.id != null) {
        print(
            '[DatabaseService] Saved category result with ID: ${insertResult.id}');
  ...
  Future<void> markAssessmentAsCompleted(
      String userId, dynamic assessmentId) async {
    if (!isConnected || _db == null) {
      print(
          '[DatabaseService] Cannot mark assessment as completed - not connected to DB');
      return;
    }
    ...
    try {
      ...
      final preAssessmentCollection =
          (await getPreAssessmentDatabase()).collection('pre-assessment');
      ...
      if (assessmentId is String && assessmentId.length == 24) {
        try {
          ...
          print(
              '[DatabaseService] Could not convert assessmentId to ObjectId: $e');
      ...
      print(
          '[DatabaseService] Updated user completion status: ${userUpdateResult.isSuccess}');
      ...
      if (assessmentUpdateResult.isSuccess) {
        print(
            '[DatabaseService] Updated pre-assessment document to track student completion');
      } else {
        print(
            '[DatabaseService] Failed to update pre-assessment document: ${assessmentUpdateResult.writeError?.errmsg ?? "Unknown error"}');
        ...
        if (assessmentObjectId == null && assessmentId is String) {
          ...
          print(
              '[DatabaseService] Alternative update approach result: ${result.isSuccess}');
        }
      }
    } catch (e) {
  ...
  Future<bool> hasStudentCompletedAssessment(
    ...
    if (!isConnected || _db == null) {
      print(
          '[DatabaseService] Cannot check completion status - not connected to DB');
      return false;
    }

    try {
      print(
          '[DatabaseService] Checking if user $userId has completed assessment $assessmentId');
      ...
      final categoryResults =
          await categoryResultCollection.find(query).toList();
      ...
      final studentResponseCollection = _db!.collection('student_responses');
      final responseQuery = where
          .eq('studentId', userIdValue)
          .and(where.eq('categoryId', assessmentId));
      final responseCount =
          await studentResponseCollection.count(responseQuery);
      if (responseCount > 0) {
      ...
      print(
          '[DatabaseService] No completion record found for user $userId and assessment $assessmentId');
      return false;
    } catch (e) {
  ...
  Future<List<String>> getCompletedAssessmentIds(String userId) async {
    if (!isConnected || _db == null) {
      print(
          '[DatabaseService] Cannot get completed assessments - not connected to DB');
      return [];
    }
    ...
    try {
      ...
      final completedAssessments =
          await preAssessmentCollection.find(query).toList();
      ...
      print(
          '[DatabaseService] Found ${completedIds.length} completed assessments for user $userId');
  ...
  Future<bool> savePreAssessmentResult({
    ...
    try {
      ...
      if (!isConnected || _db == null) {
        print(
            '[DatabaseService] Not connected to MongoDB, saved pre-assessment locally');
        return true;
      }
      ...
      print(
          '[DatabaseService] Skipping MongoDB save for assessment summary - user_responses is only for individual questions');

      // Update user profile in main database with completion status
      await updateUserPreAssessmentCompletion(
          userId, readingLevel, readingPercentage);
  ...
  Future<bool> updateUserPreAssessmentStatus(
      String userId, bool completed, String readingLevel,
      [double readingPercentage = 0.0]) async {
    // Ensure we have a valid connection before proceeding
    if (!await ensureConnection()) {
      print(
          '[DatabaseService] Cannot establish MongoDB connection for pre-assessment status update');
    ...
    try {
      print(
          '[DatabaseService] ===== UPDATING USER PRE-ASSESSMENT STATUS =====');
      ...
      try {
        ...
        print(
            '[DatabaseService] userId is not numeric, using as string: $userId');
      }
      ...
      if (userDoc == null && userIdNumeric != null) {
        print(
            '[DatabaseService] User not found with string ID, trying numeric ID...');
        userDoc =
            await usersCollection.findOne(where.eq('idNumber', userIdNumeric));
        userIdValue = userIdNumeric; // Use numeric ID for update
      }

      if (userDoc != null) {
        print(
            '[DatabaseService] Found user document with ID: ${userDoc['idNumber']}');
        print('[DatabaseService] Current user data: ${userDoc.toString()}');
      } else {
        print(
            '[DatabaseService] ERROR: User document not found for ID: $userId');
      ...
      if (result.isSuccess) {
        ...
        if (updatedDoc != null) {
          print(
              '[DatabaseService] Verification - Updated user data: ${updatedDoc.toString()}');
          print(
              '[DatabaseService] preAssessmentCompleted: ${updatedDoc['preAssessmentCompleted']}');
          print(
              '[DatabaseService] readingLevel: ${updatedDoc['readingLevel']}');
          print(
              '[DatabaseService] readingPercentage: ${updatedDoc['readingPercentage']}');
        }
      }
      ...
      print(
          '[DatabaseService] Pre-assessment status update result: ${result.isSuccess}');
  ...
  Future<void> markLessonAsCompletedAndUpdateNext(
      String userId, int lessonIndex) async {
    try {
      if (!isInitialized) {
        await initialize();
      }
      ...
      if (!await ensureConnection()) {
        print(
            '[DatabaseService] Cannot establish MongoDB connection for lesson completion update');
        return;
      }

      print(
          '[DatabaseService] Marking lesson $lessonIndex as completed and updating next lesson for user $userId');
      ...
      try {
        ...
        if (nextLessonData != null) {
          ...
          print(
              '[DatabaseService] Updated next lesson $nextLessonIndex availability in lessons collection: ${updateResult['ok'] == 1 ? 'Success' : 'Failed'}');
        } else {
          ...
          print(
              '[DatabaseService] Created new available lesson entry for lesson $nextLessonIndex: ${insertResult != null ? 'Success' : 'Failed'}');
        }
      } catch (e) {
      ...
      try {
        ...
        final assessmentQuery = where
            .eq('index', nextLessonIndex)
            .or(where.eq('lessonIndex', nextLessonIndex));
        ...
        print(
            '[DatabaseService] Updated main_assessment collection: ${updateResult['ok'] == 1 ? 'Success' : 'Failed'}');
      } catch (e) {
        print(
            '[DatabaseService] Error updating main_assessment collection: $e');
      }
      ...
      try {
        ...
        print(
            '[DatabaseService] Updated user\'s availableLessons array: ${updateResult['ok'] == 1 ? 'Success' : 'Failed'}');
      } catch (e) {
        print(
            '[DatabaseService] Error updating user\'s availableLessons array: $e');
      }

      print(
          '[DatabaseService] Successfully marked lesson $lessonIndex as completed and made lesson $nextLessonIndex available');
    } catch (e) {
      print(
          '[DatabaseService] Error in markLessonAsCompletedAndUpdateNext: $e');
      throw e;
    }
  }

// Helper method to check if a lesson was explicitly made available
  Future<bool> _isLessonExplicitlyAvailable(
      String userId, int lessonIndex) async {
    try {
      // Ensure we have a valid connection before proceeding
      if (!await ensureConnection()) {
        print(
            '[DatabaseService] Cannot check lesson availability - no MongoDB connection');
        return false;
      }
      ...
      if (userDoc != null) {
        ...
        if (availableLessons != null &&
            availableLessons.contains(lessonIndex)) {
          return true;
        }
      }
      ...
      print(
          '[DatabaseService] Error checking explicit lesson availability: $e');
      return false;
    }
  }
...
// Method to sync user's lesson completion data from MongoDB after login
  Future<void> _syncUserCompletionDataFromMongoDB(String userId) async {
    if (!isConnected || _db == null) {
      print(
          '[DatabaseService] Cannot sync completion data - not connected to MongoDB');
      return;
    }

    try {
      print(
          '[DatabaseService] Syncing completion data from MongoDB for user $userId');
      ...
      final categoryResults =
          await categoryResultCollection.find(categoryQuery).toList();

      print(
          '[DatabaseService] Found ${categoryResults.length} completed assessments in category_results');
      ...
      if (userDoc != null) {
        ...
        final completedAssessments =
            userDoc['completedAssessments'] as List<dynamic>?;
        if (completedAssessments != null && completedAssessments.isNotEmpty) {
          print(
              '[DatabaseService] Found ${completedAssessments.length} completed assessments in user document');
        }
        ...
        if (availableLessons != null && availableLessons.isNotEmpty) {
          ...
          for (final lessonIndex in availableLessons) {
            if (lessonIndex is int) {
              ...
              final existingLesson = await lessonsCollection.findOne(where
                  .eq('studentId', userId)
                  .and(where.eq('lessonIndex', lessonIndex)));
              ...
              if (existingLesson == null) {
                ...
                print(
                    '[DatabaseService] Created availability record for lesson $lessonIndex');
              }
            }
          }
        }
        ...
        for (final result in categoryResults) {
          ...
          if (category != null && categoryToLessonMap.containsKey(category)) {
            ...
            print(
                '[DatabaseService] Marked lesson $lessonIndex as completed (from category: $category)');
          }
        }
      }
      ...
      print(
          '[DatabaseService] Successfully synced completion data for user $userId');
    } catch (e) {
  ...
  Future<void> _syncAssessmentProgressFromMongoDB(String userId) async {
    if (!isConnected || _db == null) {
      print(
          '[DatabaseService] Cannot sync assessment progress - not connected to MongoDB');
      return;
    }

    try {
      print(
          '[DatabaseService] Syncing assessment progress from MongoDB for user $userId');
      ...
      final progressRecords =
          await progressCollection.find(progressQuery).toList();

      print(
          '[DatabaseService] Found ${progressRecords.length} assessment progress records in MongoDB');
      ...
      if (progressRecords.isNotEmpty && _localDb != null) {
        ...
        for (final progress in progressRecords) {
          ...
          print(
              '[DatabaseService] Synced assessment progress: $assessmentId - $currentQuestion/$totalQuestions ($progressPercentage%)');
        }
      }
    } catch (e) {
      print(
          '[DatabaseService] Error syncing assessment progress from MongoDB: $e');
  ...
  Future<bool> updateLessonAvailability(
      String userId, int lessonIndex, bool isAvailable) async {
    try {
      // Check if database is initialized
      if (!isInitialized) {
        await initialize();
      ...
      print(
          '[DatabaseService] Updating lesson $lessonIndex availability to $isAvailable for user $userId');
      ...
      if (isConnected && _db != null) {
        ...
        try {
          ...
          if (existingLesson != null) {
            ...
            print(
                '[DatabaseService] Updated lesson $lessonIndex availability in lessons collection');
          } else {
            ...
            print(
                '[DatabaseService] Created new lesson entry with availability $isAvailable');
          }
        } catch (e) {
          print('[DatabaseService] Error updating lessons collection: $e');
        }
      ...
      if (_localDb != null) {
        try {
          ...
          print(
              '[DatabaseService] Updated lesson $lessonIndex availability in local DB');
        } catch (e) {
  ...
  Future<bool> saveAssessmentResult(Map<String, dynamic> assessmentData) async {
    try {
      ...
      if (assessmentData['studentId'] != null) {
        ...
        if (currentStudentId is String) {
          try {
            ...
            print(
                '[DatabaseService] Converted string studentId "$currentStudentId" to integer $intStudentId');
          } catch (e) {
            print(
                '[DatabaseService] WARNING: Could not convert studentId to integer: $e');
          ...
          print(
              '[DatabaseService] studentId is already integer: $currentStudentId');
        } else {
          print(
              '[DatabaseService] WARNING: studentId has unexpected type: ${currentStudentId.runtimeType}');
        }
      }
      ...
      print(
          '[DatabaseService] Saving ${isPreAssessment ? "PRE" : "MAIN"} assessment for studentId: $studentId (${studentId.runtimeType})');
      ...
      if (isConnected && _db != null) {
        ...
        print(
            '[DatabaseService] Final studentId before save: ${assessmentData['studentId']} (${assessmentData['studentId'].runtimeType})');

        final result = await resultsCollection.insertOne(assessmentData);

        if (result.isSuccess) {
          print(
              '[DatabaseService] Successfully saved assessment result with integer studentId');
          print('[DatabaseService] Document ID: ${result.id}');
          return true;
        } else {
          print(
              '[DatabaseService] Failed to save: ${result.writeError?.errmsg}');
          return false;
        }
      } else {
  ...
  Future<bool> saveIndividualQuestionResponse(
    ...
    try {
      ...
      try {
        ...
        print(
            '[DatabaseService] Individual response saved to MongoDB with ID: ${result.id}');
        return true;
      } catch (e) {
        print(
            '[DatabaseService] Error saving individual response to MongoDB (will keep local copy): $e');
        return true;
      }
    } catch (e) {
  ...
  Future<bool> _saveIndividualResponseLocally(
      Map<String, dynamic> responseData) async {
    try {
      if (_localDb == null) {
        print('[DatabaseService] Local database not initialized');
    ...
    ''', [
      ...
      print(
          '[DatabaseService] Individual response saved locally for question: ${responseData['questionId']}');
      return true;
    } catch (e) {
  ...
  Map<String, dynamic> _formatResponseDataForMongoDB(
    ...
    if (category == 'Phonological Awareness') {
      ...
      if (formatted['response'] is List) {
        ...
        final formattedResponse = responses
            .map((item) => {
                  'audio': item['audio'] ?? '',
                  'match': item['match'] ?? '',
                })
...
// Helper method to save assessment result locally
  Future<bool> _saveAssessmentResultLocally(
    ...
    try {
      ...
      final assessmentDate =
          assessmentData['assessmentDate'] ?? DateTime.now().toIso8601String();
  ...
  Future<void> makeLessonAvailable(String userId, int lessonIndex) async {
    try {
      ...
      if (!isConnected) {
        print(
            '[DatabaseService] Not connected to database, cannot update lesson availability');
        return;
      }

      print(
          '[DatabaseService] Directly making lesson $lessonIndex available for user $userId');
      ...
      if (lessonDoc != null) {
        ...
        print(
            '[DatabaseService] Updated existing lesson $lessonIndex to be available');
      } else {
      ...
      try {
        ...
        print(
            '[DatabaseService] Added lesson $lessonIndex to user\'s availableLessons array');
      } catch (e) {
  ...
  Future<bool> updateUserPreAssessmentCompletion(
      String userId, String? readingLevel, double? readingPercentage) async {
    try {
      if (!isInitialized) {
        await initialize();
      }
      ...
      if (!isConnected || _db == null) {
        print(
            '[DatabaseService] Not connected to database, cannot update user completion status');
        return false;
      }
      ...
      if (updateResult.isSuccess && updateResult.nModified > 0) {
        print(
            '[DatabaseService] User completion status updated successfully for user: $userId');
        return true;
      } else {
        print(
            '[DatabaseService] No user found to update or user already has same completion status');
        return false;
      }
    } catch (e) {
...
// ADDED: Method to validate and fix category mismatches
  Future<void> validateAndFixCategoryMismatches(String userId) async {
    if (!isConnected || _db == null) {
      print(
          '[DatabaseService] Cannot validate categories - not connected to DB');
      return;
    }

    try {
      print(
          '[DatabaseService] Validating category assignments for user: $userId');
      ...
      final responses = await studentResponseCollection
      ...
      print(
          '[DatabaseService] Found ${responses.length} responses to validate');
      ...
      print(
          '[DatabaseService] Fixed $fixedResponses category mismatches for user $userId');
    } catch (e) {
...
// Helper method to determine category from question ID
  ...
  Future<bool> hasCompletedAssessment(
    ...
    try {
      ...
      if (!await ensureConnection()) {
        print(
            '[DatabaseService] Cannot establish MongoDB connection, checking local database');
        return false;
      }
      ...
      final query = where
          .eq('studentId', userIdValue)
          .and(where.eq('assessmentId', assessmentId));
      final count = await categoryResultsCollection.count(query);

      if (count > 0) {
        print(
            '[DatabaseService] Found existing assessment results for user $userId and assessment $assessmentId');
        return true;
      }
      ...
      final responseQuery = where
          ...
          .and(where.eq('categoryId', assessmentId));
      final responseCount =
          await studentResponsesCollection.count(responseQuery);

      if (responseCount > 0) {
        print(
            '[DatabaseService] Found existing student responses for user $userId and assessment $assessmentId');
        return true;
      }

      print(
          '[DatabaseService] No existing assessment results found for user $userId and assessment $assessmentId');
      return false;
    } catch (e) {
  ...
  Future<bool> isLessonCompletedEnhanced(String userId, int lessonIndex) async {
    try {
      print(
          '[DatabaseService] Enhanced check: Is lesson $lessonIndex completed for user $userId?');
      ...
      if (isConnected && _db != null) {
        try {
          ...
          if (userDoc != null) {
            ...
            if (isCompleted) {
              print(
                  '[DatabaseService] Lesson $lessonIndex completed (found in user document)');
              return true;
            }
          }
        } catch (e) {
      ...
      if (_localDb != null) {
        try {
          ...
          if (localResults.isNotEmpty) {
            print(
                '[DatabaseService] Lesson $lessonIndex completed (found in local DB)');
            return true;
          }
        } catch (e) {
      ...
      if (isConnected && _db != null) {
        try {
          ...
          final mainAssessmentResults = categoryResults
              .where((result) =>
          ...
          for (final result in mainAssessmentResults) {
            ...
            if (assessmentId.contains('ARALIN $lessonIndex') ||
              ...
              print(
                  '[DatabaseService] Lesson $lessonIndex completed (found specific main assessment result)');
              return true;
            }
          }
        } catch (e) {
  ...
  Future<void> createTestInterventionData(String userId) async {
    if (!isConnected || _db == null) {
      print(
          '[DatabaseService] Cannot create test intervention data - not connected to DB');
      return;
    }

    try {
      print(
          '[DatabaseService] Creating test intervention data for user: $userId');
      ...
      final testIntervention = {
        ...
        'updatedAt': DateTime.now().toIso8601String(),
        'assessmentId':
            'test_assessment_${DateTime.now().millisecondsSinceEpoch}',
      ...
      if (result.isSuccess) {
        ...
        print(
            '[DatabaseService] Failed to create test intervention data: ${result.writeError?.errmsg ?? "Unknown error"}');
      }
    } catch (e) {
  ...
  Future<bool> markLessonAsCompletedByCategory(
      String userId, String category) async {
    try {
      print(
          '[DatabaseService] Marking lesson as completed by category: $category for user: $userId');
      ...
      print(
          '[DatabaseService] Error marking lesson as completed by category: $e');
      return false;
    }
  }
  ...
  Future<Map<String, dynamic>> diagnoseAndFixUserDataIssues({
    ...
    try {
      print(
          '[DatabaseService] Starting comprehensive user data diagnostic for user: $userId');
      ...
      print(
          '[DatabaseService] Diagnostic and fix completed successfully for user: $userId');
    } catch (e) {
    ...
    try {
      ...
      if (!isConnected) {
        ...
        if (reconnected) {
          results['fixes_applied']
              .add('Successfully reconnected to main database');
        } else {
      ...
      try {
        ...
        if (preAssessmentDb.state != State.OPEN) {
          results['issues']
              .add('Pre_Assessment database not properly connected');
          await preAssessmentDb.open();
          results['fixes_applied']
              .add('Opened Pre_Assessment database connection');
        }
      } catch (e) {
  ...
  Future<void> _verifyUserInUsersCollection(
    ...
    try {
      ...
      if (userDoc == null) {
        ...
        if (insertResult.isSuccess) {
          results['fixes_applied']
              .add('Created user document for $userId in users collection');
        } else {
        ...
        if (currentIdType != expectedIdType && int.tryParse(userId) != null) {
          ...
          await usersCollection.updateOne(where.id(userDoc['_id']),
              modify.set('idNumber', int.parse(userId)));
          results['fixes_applied'].add(
              'Fixed idNumber data type for user $userId in users collection');
        }
      }
    } catch (e) {
  ...
  Future<void> _checkUserResponsesCollections(
      String userId, Map<String, dynamic> results) async {
    try {
      ...
      try {
        ...
        final responses = await userResponsesCollection
            .find(where
                .eq('userId', userId)
                .or(where.eq('userId', numericUserId ?? userId)))
            .toList();

        print(
            '[DatabaseService] Found ${responses.length} responses in Pre_Assessment.user_responses for user $userId');

        if (responses.isEmpty) {
          results['issues'].add(
              'No responses found in Pre_Assessment.user_responses for user $userId');
        } else {
          print('[DatabaseService] Sample response: ${responses.first}');
        }
      } catch (e) {
        results['issues']
            .add('Error checking Pre_Assessment.user_responses: $e');
      }
      ...
      try {
        ...
        final studentResponses = await studentResponsesCollection
            .find(where
                .eq('studentId', userId)
                .or(where.eq('studentId', numericUserId ?? userId)))
            .toList();

        print(
            '[DatabaseService] Found ${studentResponses.length} responses in student_responses for user $userId');

        if (studentResponses.isEmpty) {
          results['issues']
              .add('No responses found in student_responses for user $userId');
        }
      } catch (e) {
  ...
  Future<void> _fixDataTypeInconsistencies(
      String userId, Map<String, dynamic> results) async {
    try {
      final numericUserId = int.tryParse(userId);
      ...
      final userUpdateResult = await usersCollection.updateMany(
          where.eq('idNumber', userId), modify.set('idNumber', numericUserId));
      ...
      if (studentUpdateResult.isAcknowledged &&
          studentUpdateResult.nModified > 0) {
      ...
      try {
        ...
        final userResponsesCollection =
            preAssessmentDb.collection('user_responses');
        final preAssessmentUpdateResult =
            await userResponsesCollection.updateMany(where.eq('userId', userId),
                modify.set('userId', numericUserId));
        if (preAssessmentUpdateResult.isAcknowledged &&
            preAssessmentUpdateResult.nModified > 0) {
          fixesApplied += preAssessmentUpdateResult.nModified;
        }

        if (fixesApplied > 0) {
          results['fixes_applied'].add(
              'Fixed data type inconsistencies for user $userId across $fixesApplied documents');
        }
      } catch (e) {
        results['issues'].add(
            'Error fixing data types in Pre_Assessment.user_responses: $e');
      }
    } catch (e) {
  ...
  Future<void> _syncDataBetweenCollections(
      String userId, Map<String, dynamic> results) async {
    try {
      ...
      final usersCollection = getCollection('users');
      final userDoc =
          await usersCollection.findOne(where.eq('idNumber', numericUserId));

      if (userDoc != null) {
        // Check if user has assessment responses but no reading level
        if (userDoc['readingLevel'] == null ||
            userDoc['preAssessmentCompleted'] != true) {
          ...
          try {
            ...
            final assessmentResults = await userResponsesCollection
                .find(where.eq('userId', numericUserId))
                .toList();
              ...
              await usersCollection.updateOne(
                  ...
                  modify
                      ...
                      .set('readingPercentage',
                          latestResult['readingPercentage'])
                      .set('lastAssessmentDate', latestResult['createdAt'])
                      .set('updatedAt', DateTime.now().toIso8601String()));

              results['fixes_applied'].add(
                  'Synced assessment data to user document for user $userId');
            }
          } catch (e) {
  ...
  Future<Map<String, dynamic>?> fetchUserData(String userId) async {
    try {
      ...
      final usersCollection = getCollection('users');
      final userDoc =
          await usersCollection.findOne(where.eq('idNumber', normalizedUserId));
  ...
  Future<List<Map<String, dynamic>>> fetchUserResponses(String userId) async {
    try {
      ...
      final responses = await userResponsesCollection
          .find(where.eq('userId', normalizedUserId))
          .toList();
  ...
  Future<List<Map<String, dynamic>>> fetchStudentResponses(
    ...
    try {
      ...
      final responses = await studentResponsesCollection
          .find(where.eq('studentId', normalizedStudentId))
          .toList();
  ...
  Future<bool> upsertUserProfile({
    ...
    try {
      ...
      if (preAssessmentCompleted != null)
        userData['preAssessmentCompleted'] = preAssessmentCompleted;
      if (readingPercentage != null)
        userData['readingPercentage'] = readingPercentage;
      ...
      print(
          '[DatabaseService] User profile upsert result: ${result.isAcknowledged}');
  ...
  Future<void> saveLessonProgress(
    ...
    try {
      print(
          '[DatabaseService] Saving lesson progress: User=$userId, Lesson=$lessonIndex, Progress=$currentQuestion/$totalQuestions ($progressPercentage%)');
  ...
  Future<void> saveAssessmentProgress(String userId, String assessmentId,
      int currentQuestion, int totalQuestions, int progressPercentage) async {
    try {
      print(
          '[DatabaseService] Saving assessment progress: User=$userId, Assessment=$assessmentId, Progress=$currentQuestion/$totalQuestions ($progressPercentage%)');
  ...
  Future<Map<String, dynamic>?> getLessonProgress(
    ...
    try {
      ...
      if (_db != null) {
        final progressCollection = _db!.collection('lesson_progress');
        final progressDoc = await progressCollection.findOne(where
            .eq('userId', userId)
            .and(where.eq('lessonIndex', lessonIndex)));
  ...
  Future<Map<String, dynamic>?> loadAssessmentByCategory({
    ...
    try {
      print(
          '[DatabaseService] Loading assessment for category: $category, readingLevel: $readingLevel');
      ...
      print(
          '[DatabaseService] Found ${categoryQuestions.length} questions for category: $category');
  ...
  Future<List<Map<String, dynamic>>> loadQuestionsByCategory({
    ...
    try {
      print(
          '[DatabaseService] Loading questions for category: $category, readingLevel: $readingLevel');
      ...
      print(
          '[DatabaseService] Loaded ${categoryQuestions.length} questions for category: $category');
  ...
  Future<bool> repairPA001Data() async {
    try {
      ...
      final currentQuestionSet =
          pa001Question['questionSet'] as Map<String, dynamic>?;
      if (currentQuestionSet != null) {
        final currentAudioTexts =
            currentQuestionSet['audioTexts'] as List<dynamic>?;
        final currentMatchingOptions =
            currentQuestionSet['matchingOptions'] as List<dynamic>?;

        print(
            '[DatabaseService] Current audioTexts: $currentAudioTexts (length: ${currentAudioTexts?.length})');
        print(
            '[DatabaseService] Current matchingOptions: $currentMatchingOptions (length: ${currentMatchingOptions?.length})');

        // If data is already complete (5 items), no need to repair
        if (currentAudioTexts?.length == 5 &&
            currentMatchingOptions?.length == 5) {
          print(
              '[DatabaseService] PA_001 data is already complete - no repair needed');
          return true;
        }
      }
      ...
      print(
          '[DatabaseService] Updating PA_001 with complete data: $completeQuestionSet');
      ...
      if (updateResult.isSuccess) {
        ...
        print(
            '[DatabaseService] Updated audioTexts: ${completeQuestionSet['audioTexts']}');
        print(
            '[DatabaseService] Updated matchingOptions: ${completeQuestionSet['matchingOptions']}');
        print(
            '[DatabaseService] Updated correctPairs: ${completeQuestionSet['correctPairs']}');
        return true;
      } else {
        print(
            '[DatabaseService] ❌ Failed to update PA_001: ${updateResult.writeError?.errmsg}');
        return false;
      }
    } catch (e) {
  ...
  Future<bool> repairPA002Data() async {
    try {
      ...
      // Check current questionSet
      final currentQuestionSet =
          pa002Question['questionSet'] as Map<String, dynamic>?;
      if (currentQuestionSet != null) {
        final currentAudioTexts =
            currentQuestionSet['audioTexts'] as List<dynamic>?;
        final currentMatchingOptions =
        ...
        if (currentAudioTexts?.length == 5 &&
            currentMatchingOptions?.length == 5) {
          print(
              '[DatabaseService] PA_002 data is already complete - no repair needed');
          return true;
        }
      }
      ...
      print(
          '[DatabaseService] Updating PA_002 with complete data: $completeQuestionSet');
      ...
      if (updateResult.isSuccess) {
        ...
        print(
            '[DatabaseService] ❌ Failed to update PA_002: ${updateResult.writeError?.errmsg}');
        return false;
      }
    } catch (e) {
  ...
  Future<bool> repairPA003Data() async {
    try {
      ...
      // Check current questionSet
      final currentQuestionSet =
          pa003Question['questionSet'] as Map<String, dynamic>?;
      if (currentQuestionSet != null) {
        final currentAudioTexts =
            currentQuestionSet['audioTexts'] as List<dynamic>?;
        final currentMatchingOptions =
        ...
        if (currentAudioTexts?.length == 5 &&
            currentMatchingOptions?.length == 5) {
          print(
              '[DatabaseService] PA_003 data is already complete - no repair needed');
          return true;
        }
      }
      ...
      print(
          '[DatabaseService] Updating PA_003 with complete data: $completeQuestionSet');
      ...
      if (updateResult.isSuccess) {
        ...
        print(
            '[DatabaseService] ❌ Failed to update PA_003: ${updateResult.writeError?.errmsg}');
        return false;
      }
    } catch (e) {
  ...
  String _extractCategoryFromQuestion(Map<String, dynamic> question) {
    ...
    if (questionId.startsWith('PRE_AK') ||
        questionId.contains('_AK_') ||
        questionTypeId.contains('AK')) {
      return 'Alphabet Knowledge';
    } else if (questionId.startsWith('PRE_PA') ||
        questionId.contains('_PA_') ||
        questionTypeId.contains('PA')) {
      return 'Phonological Awareness';
    } else if (questionId.startsWith('PRE_DC') ||
        questionId.contains('_DC_') ||
        questionTypeId.contains('DC')) {
      return 'Decoding';
    } else if (questionId.startsWith('PRE_WR') ||
        questionId.contains('_WR_') ||
        questionTypeId.contains('WR')) {
      return 'Word Recognition';
    } else if (questionId.startsWith('PRE_RC') ||
        questionId.contains('_RC_') ||
        questionTypeId.contains('RC')) {
      return 'Reading Comprehension';
    }
    ...
    if (questionType.contains('alphabet')) {
      ...
      return 'Word Recognition';
    } else if (questionType.contains('comprehension') ||
        questionType.contains('reading')) {
      return 'Reading Comprehension';
    }
...
List<Map<String, dynamic>> getOptions(Map<String, dynamic> question) {
  if (question['options'] != null && (question['options'] as List).isNotEmpty) {
  ...
  } else if (question['sentenceQuestions'] != null &&
      (question['sentenceQuestions'] as List).isNotEmpty) {
    ...
    if (correct.isNotEmpty)
      options.add({'optionText': correct, 'isCorrect': true});
    if (incorrect.isNotEmpty)
      options.add({'optionText': incorrect, 'isCorrect': false});
    // Optionally shuffle
    options.shuffle();
    return options;
  }
