...
class TTSProvider extends ChangeNotifier {
  ...
  Map<String, String>? _findVoiceByNameContains(String nameContains) {
    try {
      return _availableVoices.firstWhere(
        (voice) => (voice['name']?.toLowerCase().contains(nameContains.toLowerCase()) ?? false) ||
                  (voice['voiceName']?.toLowerCase().contains(nameContains.toLowerCase()) ?? false),
      );
    } catch (e) {
      return null;
    }
  ...
  Map<String, String>? _findVoiceByLanguage(String langCode) {
    try {
      return _availableVoices.firstWhere(
        (voice) => (voice['locale']?.toLowerCase().contains(langCode.toLowerCase()) ?? false) ||
                  (voice['language']?.toLowerCase().contains(langCode.toLowerCase()) ?? false),
      );
    } catch (e) {
      return null;
    }
