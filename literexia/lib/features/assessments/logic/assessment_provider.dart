...
class AssessmentProvider extends ChangeNotifier {
  ...
  void _storeRawQuestionData(Assessment assessment) {
    print('[AssessmentProvider] Storing raw question data for ${assessment.questions.length} questions');
    ...
    for (final question in assessment.questions) {
      ...
      print('[AssessmentProvider] Stored data for question ${question.questionId}:');
      print('  - Passages: ${question.passages?.length ?? 0}');
      print('  - Sentence Questions: ${question.sentenceQuestions?.length ?? 0}');
    }
  }
  ...
  Future<void> loadPreAssessment() async {
    try {
      ...
      if (assessment != null) {
        ...
        print('[AssessmentProvider] Successfully loaded PRE-ASSESSMENT: ${assessment.title}');
        ...
        print('[AssessmentProvider] Marked as PRE-ASSESSMENT: $_isPreAssessment');
        
        notifyListeners();
      } else {
        print('[AssessmentProvider] Failed to load pre-assessment - no data returned');
  ...
  Future<void> loadMainAssessment(dynamic assessmentId, {String? readingLevel}) async {
    try {
      ...
      final assessment = await _repository.getMainAssessment(assessmentId, readingLevel: targetReadingLevel);
  ...
  Future<void> loadAssessmentWithReadingLevel(dynamic assessmentId, String userReadingLevel) async {
    print('[AssessmentProvider] Loading assessment with reading level context');
    print('[AssessmentProvider] Assessment ID: $assessmentId, User Reading Level: $userReadingLevel');
  ...
  void _determineReadingLevelFromPreAssessment() {
    ...
    print('[AssessmentProvider] PRE-ASSESSMENT - Overall percentage: $overallPercentage%');
  ...
  void _determineReadingLevelFromMainAssessment() {
    ...
    print('[AssessmentProvider] Determining reading level from MAIN ASSESSMENT');
    ...
    print('[AssessmentProvider] MAIN ASSESSMENT - Category scores: $categoryPercentages');
    print('[AssessmentProvider] MAIN ASSESSMENT - Overall percentage: $overallPercentage%');
    print('[AssessmentProvider] MAIN ASSESSMENT - Determined reading level: $_readingLevel');
  }
  ...
  Future<void> saveResults(String userId) async {
    try {
      ...
      print('[AssessmentProvider] Assessment Type: ${_isPreAssessment ? "PRE-ASSESSMENT" : "MAIN ASSESSMENT"}');
      print('[AssessmentProvider] Reading Level: ${_readingLevel ?? "Undefined"}');
      ...
      if (_isPreAssessment) {
        print('[AssessmentProvider] ===== ROUTING TO PRE-ASSESSMENT SAVE =====');
        ...
        print('[AssessmentProvider] ===== ROUTING TO MAIN ASSESSMENT SAVE =====');
      ...
      if (saveSuccess) {
        print('[AssessmentProvider] ===== SUCCESSFULLY SAVED ${_isPreAssessment ? "PRE" : "MAIN"} ASSESSMENT RESULTS =====');
        
        // IMPORTANT: For pre-assessments, also update user profile
        if (_isPreAssessment) {
          print('[AssessmentProvider] Updating user profile for pre-assessment completion');
        ...
        throw Exception('Failed to save ${_isPreAssessment ? "pre" : "main"} assessment results');
      }
    } catch (e) {
  ...
  Future<void> saveDetailedResults(String userId, String assessmentId) async {
    ...
    try {
      ...
      if (_isPreAssessment) {
        print('[AssessmentProvider] ===== SAVING DETAILED PRE-ASSESSMENT RESULTS =====');
        ...
        if (success) {
          print('[AssessmentProvider] Pre-assessment detailed results saved successfully');
        } else {
          throw Exception('Failed to save detailed pre-assessment results');
        }
        
      } else {
        print('[AssessmentProvider] ===== SAVING DETAILED MAIN ASSESSMENT RESULTS =====');
        ...
        print('[AssessmentProvider] Main assessment detailed results saved successfully');
      }
    } catch (e) {
